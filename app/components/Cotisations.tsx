...
const OPERATEURS = [
  ...
  },
  {
    ...
    tip: "Remplacez NUMERO_MOOV par le numero Moov du beneficiaire quand disponible",
  },
  {
    nom: "Wave",
    couleur: "#0891b2",
    ...
    tip: "Composez ce code ou ouvrez Wave. Motif : Cotisation JN1000 + votre nom",
  },
];
...
export default function Cotisations() {
  ...
  const copier = (texte: string, cle: string) => {
    navigator.clipboard.writeText(texte).then(() => {
      ...
      afficherToast("Copie dans le presse-papiers !");
    });
  };
  ...
  const togglePaiement = (id: string, paye: boolean) => {
    updateAdherent(id, {
      paye: !paye,
      date_paiement: !paye ? new Date().toISOString().split("T")[0] : "",
    });
    afficherToast(!paye ? "Paiement confirme !" : "Paiement annule");
  };
  ...
  const filtres = adherents.filter(a => {
    ...
    const ok = (a.nom + " " + a.prenoms).toLowerCase().includes(q)
    ...
    <div>
      ...
      <div style={{ marginBottom: "1.75rem" }}>
        ...
        <p style={{ color: "var(--texte-sec)", fontSize: 13.5, marginLeft: 16 }}>
          Gestion des paiements · 1 000 FCFA par adherent
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(155px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
        {[
          ...
          { titre: "Collecte",          val: montant.toLocaleString("fr") + " F",             couleur: "var(--vert)",      fond: "var(--vert-pale)" },
          { titre: "Reste a collecter", val: (nonPayes * 1000).toLocaleString("fr") + " F",   couleur: "var(--rouge)",     fond: "var(--rouge-pale)" },
      ...
      <div className="carte" style={{ marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 12 }}>
          <span style={{ color: "var(--vert)", fontWeight: 600 }}>
            {payes} paye{payes > 1 ? "s" : ""}
          </span>
          <span style={{ color: "var(--alerte)", fontWeight: 600 }}>
            {nonPayes} en attente
          </span>
        </div>
      </div>
      ...
      <div className="carte" style={{ marginBottom: "1.5rem", borderLeft: "4px solid var(--vert)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: "1.25rem" }}>
          ...
          <h2 style={{ fontWeight: 700, fontSize: 15, color: "var(--texte)" }}>
            Instructions de paiement — 1 000 FCFA
          </h2>
        </div>
          ...
          {OPERATEURS.map(op => (
            <div key={op.nom} style={{ border: "1.5px solid " + op.bordure, borderRadius: 14, padding: "14px 16px", background: op.fond }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <button
                  onClick={() => copier(op.code, op.nom)}
                  style={{
                    border: "1px solid " + (copie === op.nom ? "rgba(22,163,74,0.4)" : op.bordure),
                    background: copie === op.nom ? "rgba(22,163,74,0.10)" : "white",
                    color: copie === op.nom ? "#16a34a" : op.couleur,
                    borderRadius: 8,
                    padding: "5px 12px",
                    fontSize: 12,
                    fontWeight: 600,
              ...
              <div style={{ fontSize: 12.5, color: "var(--texte-sec)", lineHeight: 1.6 }}>
                {op.tip}
              </div>
            </div>
          ))}
        ...
        <div style={{ padding: "1rem 1.5rem", borderBottom: "1px solid var(--bordure)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, background: "var(--vert-pale)" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <select
              ...
              style={{ width: "auto" }}
              value={filtre}
              onChange={e => setFiltre(e.target.value as "tous"|"paye"|"impaye")}
            >
                ...
                <tr key={a.id}>
                  ...
                  <td>
                    <span
                      className="badge"
                      style={{
                        background: a.paye ? "var(--vert-pale)" : "var(--alerte-pale)",
                        color: a.paye ? "var(--vert)" : "var(--alerte)",
                        border: "1px solid " + (a.paye ? "var(--vert-clair)" : "rgba(180,83,9,0.2)"),
                        display: "inline-flex",
                        alignItems: "center",
                        gap: 5,
                      }}
                  ...
                  <td>
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 600,
                        cursor: "pointer",
                        background: a.paye ? "var(--rouge-pale)" : "var(--vert-pale)",
                        color: a.paye ? "var(--rouge)" : "var(--vert)",
                      }}
                    >
      ...
      {toast && (
        <div className="toast">
          <span style={{ color: "var(--vert)" }}>✓</span> {toast}
        </div>
      )}
    </div>
  );
